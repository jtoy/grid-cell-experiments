...
_DATASETS = dict(
    square_room=DatasetInfo(
        basepath="square_room_100steps_2.2m_1000000", size=100, sequence_length=100, coord_range=((-1.1, 1.1), (-1.1, 1.1))
    ),
)
...
class DataReader(object):
    ...
    def __init__(
        ...
        if dataset not in _DATASETS:
            raise ValueError("Unrecognized dataset {} requested. Available datasets " "are {}".format(dataset, _DATASETS.keys()))
    ...
    def _make_read_op(self, reader, filename_queue):
        ...
        feature_map = {
            "init_pos": tf.compat.v1.FixedLenFeature(shape=[2], dtype=tf.float32),
            "init_hd": tf.compat.v1.FixedLenFeature(shape=[1], dtype=tf.float32),
            "ego_vel": tf.compat.v1.FixedLenFeature(shape=[self._dataset_info.sequence_length, 3], dtype=tf.float32),
            "target_pos": tf.compat.v1.FixedLenFeature(shape=[self._dataset_info.sequence_length, 2], dtype=tf.float32),
            "target_hd": tf.compat.v1.FixedLenFeature(shape=[self._dataset_info.sequence_length, 1], dtype=tf.float32),
        }
